...
# Defaults
#
# 30 Mb for the boot partition
BOOT_SIZE=${BOOT_SIZE:-30}

# 500 Mb for the rootfs partition
ROOTFS_SIZE=${ROOTFS_SIZE:-500}

# 5% for swap
#SWAP_RATIO=5
# 200MB for swap
SWAP_SIZE=${SWAP_SIZE:-200}
...
GRUB_MODULES=$(dirname ${HDDIMG})/grub-modules-*.tgz
...
[ -z "$SWAP_SIZE" ] && SWAP_SIZE=$((DEVICE_SIZE*SWAP_RATIO/100))
[ -z "$ROOTFS_SIZE" ] && ROOTFS_SIZE=$((DEVICE_SIZE-BOOT_SIZE-SWAP_SIZE))
...
cp -f $HDDIMG_MNT/xen.gz $BOOTFS_MNT >$OUT 2>&1 || error "Failed to copy xen.gz"
cp -f $HDDIMG_MNT/xen.efi $BOOTFS_MNT >$OUT 2>&1 || error "Failed to copy xen.efi"
cp -f $HDDIMG_MNT/xen.cfg $BOOTFS_MNT >$OUT 2>&1 || error "Failed to copy xen.cfg"
...
if [ -e "$GRUB_CFG" ]; then
	...
	sed -i "s@rootwait@@" $GRUB_CFG
	sed -i "s@loglevel=[0-9]@@" $GRUB_CFG
	sed -i "s@root=/dev/ram0\b@root=$TARGET_ROOTFS rootwait loglevel=7 @" $GRUB_CFG

	for x in ${GRUB_MODULES}; do
		tar xf "$x" --no-same-owner -C "${EFIDIR}" "*/*.mod"
	done
fi

XEN_CFG="$BOOTFS_MNT/xen.cfg"
if [ -e "$XEN_CFG" ]; then
	info "Configuring XEN"
	sed -i "s@rootwait@@" $XEN_CFG
	sed -i "s@loglevel=[0-9]@@" $XEN_CFG
	sed -i "s@root=/dev/ram0\b@root=$TARGET_ROOTFS rootwait loglevel=7 @" $XEN_CFG
fi
